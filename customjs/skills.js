...
Animate.prototype.options = {
  density: 15, // Affects how many poitns are created
...
Animate.prototype.animate = function() {
  ...
  for(i in this.points) {
    if(this.points[i].distanceTo(this.target, true) < 5000) {
       ...
       this.points[i].opacity = 0.025;
       this.points[i].circle.opacity = 0.025;
    }
  }
   // Clear
...
Animate.prototype.mousemove = function(event) {
  if(event.offsetX || event.offsetY) {
    this.target.position.x = event.offsetX;
    this.target.position.y = event.offsetY;
